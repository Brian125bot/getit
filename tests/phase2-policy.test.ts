...
test('Phase 2 local .getitignore blocks matching reads and writes', async () => {
  ...
  const result = await validatePath(pem, { cwd: dir });
  assert.strictEqual(result.allowed, false);
  assert.ok(result.reason?.includes('policy'));
});

test('Phase 2 override still blocks catastrophic paths', async () => {
  configureRuntimeSession({ policyProfile: 'override' });
  const result = await validatePath('/proc/cpuinfo', { cwd: process.cwd(), profile: 'override' });
  assert.strictEqual(result.allowed, false);
});
